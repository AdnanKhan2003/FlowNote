...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  return (
    <div className="container animate-fade">
      <Header userEmail={user.email} onCreateNote={handleCreateNote} onLogout={logout} />

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '32px' }}>
        <main>
          <div style={{ position: 'relative', marginBottom: '24px' }}>
            <Search style={{ position: 'absolute', left: '12px', top: '22px', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} size={18} />
            <Input 
              type="text" 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
            {filteredNotes.map(note => (
               <NoteCard 
                 ...
                 onDeleteClick={(e) => { e.stopPropagation(); setIsDeleting(note.id); }} 
               />
            ))}
          </div>
        </main>
