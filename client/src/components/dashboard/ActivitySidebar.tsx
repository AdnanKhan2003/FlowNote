...
const ActivitySidebar: React.FC<ActivitySidebarProps> = ({ activities, userEmail }) => {
  return (
    <aside>
          ...
          {activities.map(act => (
            <div key={act.id} style={{ fontSize: '13px' }}>
              <div style={{ color: 'var(--text)' }}>
                <span style={{ fontWeight: '600', color: 'var(--primary)' }}>{act.userEmail === userEmail ? 'You' : act.userEmail}</span> {act.action.toLowerCase()}d <strong>{act.noteTitle || 'a note'}</strong>
              </div>
