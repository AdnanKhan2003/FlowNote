...
const AuthCard: React.FC<AuthCardProps> = ({ title, error, children, footer }) => {
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
      <div className="glass-card animate-fade" style={{ padding: '40px', width: '100%', maxWidth: '400px' }}>
        ...
        {children}
        {footer && <div style={{ marginTop: '24px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '14px' }}>{footer}</div>}
      </div>
    </div>
  );
};
